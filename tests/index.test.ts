...
describe('instantiate client', () => {
  ...
  test('maxRetries option is correctly set', () => {
    const client = new SFCNodes({ maxRetries: 0, bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['SFC_NODES_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new SFCNodes();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['SFC_NODES_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new SFCNodes({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
